...
|Resolved a regression in Quinn Update 4 that makes the Defender set iptables/nftables entries in your environment, even when there is no policy such as Cloud Native Network Security (CNNS) or DNS monitoring requesting it.

This bug affects all 34.04 Defenders. In the case of a Defender running on a machine that has an active policy such as Telephony Application Server (TAS) or CNNS or DNS monitoring, this may lead to severe connectivity issues.
