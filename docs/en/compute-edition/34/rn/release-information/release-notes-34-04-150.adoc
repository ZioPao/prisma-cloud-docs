:toc: macro
== 34.04.1XX Hotfix Release Notes
...
* Improved Startup Reliability: Resolved a race condition in CRI socket discovery during boot that could occasionally prevent the Defender from initializing.
...
|Added ability to restrict UI access for access key logins under a feature flag, improving security posture for programmatic access.
...
Suppressed CVE-2026-33186 in the gRPC vendor dependency via database migration.
